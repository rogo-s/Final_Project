...
class SearchToAdapter(private val context: Context, private val listairport : List<Flight>) : RecyclerView.Adapter<SearchToAdapter.ViewHolder>() {
    ...
    class ViewHolder (val binding: ItemDestinasiBinding) : RecyclerView.ViewHolder(binding.root) {
        fun bindSearch(item: Flight, sharedPreferences: SharedPreferences) {
            binding.tvDestination.setOnClickListener {
                ...
                editor.putString("keyTo", item.arrivalCity)
                editor.apply()

                val navController = Navigation.findNavController(binding.root)
                navController.previousBackStackEntry?.savedStateHandle?.set("selected_destination_to", item.arrivalCity)
                navController.navigate(R.id.action_pencarianToFragment_to_homeFragment2)
            }
        }
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.binding.tvDestination.text = item.arrivalCity
        ...
        holder.itemView.setOnClickListener {
            ...
            Navigation.findNavController(it).navigate(R.id.action_pencarianToFragment_to_homeFragment2)
        }
